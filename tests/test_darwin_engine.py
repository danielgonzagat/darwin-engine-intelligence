...
from core.darwin_engine_real import DarwinEngine, ReproductionEngine
...
class TestEvolvableMNIST:
    ...
    def test_mnist_build(self):
        """Teste #4: Construção de modelo"""
        individual = EvolvableMNIST({'hidden_size': 128, 'num_layers': 2})
        model = individual.build()
        assert model is not None
        
        # Contar parâmetros
        params = sum(p.numel() for p in model.parameters())
        assert params > 100000  # Pelo menos 100k params
