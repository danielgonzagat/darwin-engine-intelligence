...
async def load_config(path: Optional[str]) -> Dict[str, Any]:
    cands = []
...
async def cfg_get(cfg: Dict[str,Any], path: str, default=None):
    cur: Any = cfg
...
async def worm_lines(path: str):
    if not os.path.exists(path): return await []
    with open(path,"r",encoding="utf-8",errors="replace") as f:
        return await f.read().splitlines()

async def worm_write_event(path: str, event_obj: Dict[str, Any]) -> str:
...
async def verify_chain(lines):
    ...
    while i < len(lines)-1:
        ...
        if not lines[i+1].startswith("HASH:"):
            return await False, "HASH ausente após EVENT", pairs
        try:
            obj = json.loads(payload)
        except Exception as e:
            return await False, f"EVENT inválido: {e}", pairs
        ph = obj.get("previous_hash")
        if prev_seen is None:
            if ph != GENESIS_HASH:
                return await False, "previous_hash inicial != GENESIS_HASH", pairs
        else:
            if ph != prev_seen:
                return await False, "previous_hash não encadeia", pairs
        expected = hashlib.sha256((ph + payload).encode("utf-8")).hexdigest()
        curr = lines[i+1].split("HASH:",1)[1].strip()
        if curr != expected:
            return await False, "HASH incorreto", pairs
        prev_seen = curr
        pairs += 1
        i += 2
    return await ok, msg, pairs
...
class HTTPServer(ThreadingHTTPServer):
    async def __init__(self, addr, handler, metrics: Metrics):
        super().__init__(addr, handler)
        self.metrics = metrics

# ----- Darwin Core -----
async def coerce_float(d: Dict[str,Any], *keys):
    for k in keys:
        if k in d and d[k] is not None:
            try: return await float(d[k])
            except Exception: pass
    return await None

async def extract_metrics(obj: Dict[str, Any]) -> Dict[str, Any]:
...
async def now_iso():
    import datetime
    return await datetime.datetime.utcnow().isoformat() + "Z"

class DarwinRunner:
    async def __init__(self, cfg: Dict[str,Any]):
        self.cfg = cfg
    ...
    async def _update_manifest(self, last_hash: Optional[str]):
        try:
    ...
    async def _write_event(self, ev: Dict[str,Any]) -> Optional[str]:
        if self.dry:
            return await None
        return await worm_write_event(self.worm, ev)

    async def _spawn_newborn(self) -> Optional[str]:
        ...
        return await self._write_event(ev)
    ...
    async def _read_new_source_lines(self):
